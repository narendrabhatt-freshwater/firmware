...
[`cmi_core/examples/sdk/`](cmi_core/examples/sdk/README.md).
...
**Performance CLI** — arrange a sound as `channel.be`, a
`samples/` bank, and optional `wavetables/osc0...osc7` files, then run:

```bash
fw cli --sample /path/to/sample.wav --root-hz 261.625565

# Or select another project:
fw play my-sound --rs485 /dev/cu.usbserial-0001
```

`fw cli` uses the repository's example script and a sample supplied by the
user; `fw play` supports complete banks. Both auto-detect the connected
...
[`cmi_core/tools/cmi_play/README.md`](cmi_core/tools/cmi_play/README.md).
No sample or wavetable asset is bundled or generated by the CLI.
