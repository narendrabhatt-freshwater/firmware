...
Hot-path DSP stays **fixed-point Q31** (project rule). The FPU is used only
on the cold path when recomputing coefficients from a cutoff change.
...
| Choice | Reason |
|--------|--------|
| 4-pole Butterworth | Requested response: maximally flat passband, −80 dB/decade roll-off |
| Two cascaded biquads (SOS) | A 4th-order IIR = two 2nd-order sections; numerically stabler than one 4th-order DF |
| Same cutoff on both sections | Classic Butterworth cascade (matched `fc`, different `Q`) |
