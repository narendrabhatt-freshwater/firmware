...
protocol::Result Bus::Open(const std::string &path, const BusOptions &opts)
{
  Close();
  opts_ = opts;
  if (opts_.atten_db > 127u) {
    last_ = protocol::Result::LocalErr("range", "atten_db must be 0..127");
    return last_;
  }

  port_.SetManualRtsControl(opts_.manual_rts);
  if (!port_.Open(path, opts_.baud)) {
    last_ = protocol::Result::IoErr(port_.LastError().c_str());
    return last_;
  }
  path_ = path;
  ...
  channel_ = std::make_unique<protocol::ChannelClient>(*transport_);
  effect_ = std::make_unique<protocol::EffectClient>(*transport_);

  if (opts_.effect_echo != EffectEcho::Leave) {
    const bool want_on = (opts_.effect_echo == EffectEcho::On);
    protocol::Result echo = effect_->SetEcho(want_on);
    last_ = echo;
    if (!echo.ok()) {
  ...
  auto fail_close = [this](const protocol::Result &r) -> protocol::Result {
    last_ = r;
    bus_fault_ = true;
    open_ = false;
    channel_.reset();
  ...
  open_ = true;
  bus_fault_ = false;
  last_.status = protocol::Status::Ok;
  return last_;
}
...
bool Bus::SoftRecover()
{
  ...
  lo.retries = 2;
  link_->SetOptions(lo);
  const protocol::Result sil = channel_->AllNotesOff();
...
void Bus::ForceClearBus()
{
  ...
  port_.FlushInput();
  (void)transport_->SendBlind(protocol::Target::Channel, "n 0");
  (void)transport_->SendBlind(protocol::Target::Channel, "n 0");
  port_.FlushInput();
}
...
protocol::Result Bus::Exec(protocol::Target target, const std::string &command)
{
  if (!open_ || !transport_) {
    last_ = protocol::Result::IoErr("bus not open");
    return last_;
  }
  last_ = transport_->Exchange(target, command);
  if (last_.status == protocol::Status::IoError) {
    bus_fault_ = true;
  }
  return last_;
}
