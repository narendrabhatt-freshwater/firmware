# protocol — Freshwater console wire API
...
```cmake
add_subdirectory(path/to/libs/protocol)
target_link_libraries(your_app PRIVATE protocol::protocol)
```

C++17. Header root: `libs/protocol/include` → `#include "protocol/…"`.
...
std::string cmd = protocol::FormatSetNote(0, 261.625565);
...
struct MyTransport : protocol::IConsoleTransport {
  protocol::Result Exchange(protocol::Target t,
                            const std::string &command) override {
    // 1) Optionally prepend protocol::TargetPrefix(t)
    // 2) Write command + '\r'
    // 3) Read one reply line
    // 4) return protocol::ParseReplyBody(line, t);
  }
  bool SendBlind(protocol::Target t, const std::string &command) override {
...
MyTransport tx;
protocol::ChannelClient ch(tx);
protocol::Result r = ch.SetNote(0, 440.0);
...
| Header | Contents |
| ------ | -------- |
| `protocol/types.hpp` | `Target`, `Status`, `TargetPrefix` |
| `protocol/result.hpp` | `Result`, `ParseReplyBody` |
| `protocol/transport.hpp` | `IConsoleTransport` |
