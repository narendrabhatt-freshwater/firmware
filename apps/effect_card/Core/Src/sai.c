...
void MX_SAI1_Init(void)
{
  ...
  /* NOTE on rates: an earlier "bus runs at half rate" diagnosis was
   * WRONG — it came from reading a 500/s callback rate while
   * SAI_FRAMES_PER_HALF was 192 (2 ms halves), which is the correct rate
   * for a true 96 kHz.  The 192K override that appeared to fix it really
   * ran the bus at 192 kHz into a 96 kHz USB stream (FIFO overflow,
   * dropped chunks, beating).  AudioFrequency stays at the generated 96K,
   * with SAI_FRAMES_PER_HALF = 96 in main.c so the DMA half-buffer (1 ms)
   * matches the USB frame interval one-for-one. */
