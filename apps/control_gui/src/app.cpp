...
/** Playback-mode pill for the Tone header strip. */
bool ModeBtn(const char *label, bool selected)
{
  ImFont *font = fw::theme::g_fonts.caps;
  ImGui::PushFont(font);
  const ImVec2 ts = ImGui::CalcTextSize(label);
  const ImVec2 size(ts.x + S(24.f), ts.y + S(7.f));
  const ImVec2 p0 = ImGui::GetCursorScreenPos();
  ImGui::PushID(label);
  ImGui::InvisibleButton("##mode", size);
  const bool clicked = ImGui::IsItemClicked();
  const bool hovered = ImGui::IsItemHovered();
  ImGui::PopID();
  ImDrawList *dl = ImGui::GetWindowDrawList();
  ...
  if (selected) {
    dl->AddRectFilled(p0, p1, fw::theme::U32A(kPalette.accent, 0.12f), S(2.f));
    dl->AddRect(p0, p1, fw::theme::U32A(kPalette.accent, 0.30f), S(2.f));
  } else if (hovered) {
    dl->AddRectFilled(p0, p1, fw::theme::U32(kPalette.panel_alt), S(2.f));
  }
  dl->AddText(ImVec2(p0.x + S(12.f), p0.y + S(3.5f)),
              fw::theme::U32(selected ? kPalette.accent : kPalette.text_dim),
              label);
  ImGui::PopFont();
  return clicked;
}
...
void App::SendConsole(const char *cmd)
{
  ...
  const char tletter = raw_target == 1 ? 'E' : raw_target == 2 ? 'A' : 'C';
  log.Push(LogKind::Tx,
...
void App::DrawStatusBar()
{
  ...
  // MODE chip — click toggles Notes ↔ Wave
  {
    ChipSeg segs[3];
    segs[0] = {"MODE", kPalette.text_dim, fs};
    segs[1] = {play_mode == 1 ? "WAVE" : "NOTES", kPalette.text, fm};
    segs[2] = {"\u2191\u2193", kPalette.muted, fs};
    if (StatusChip("mode_chip", false, kPalette.muted, false, segs, 3, false,
                   kPalette.border, false, false)) {
      play_mode = play_mode == 1 ? 0 : 1;
      if (bus.IsOpen()) {
        bus.QueueMode(play_mode == 1 ? protocol::PlayMode::Wave
                                     : protocol::PlayMode::Notes);
      }
      log.Push(std::string("Playback mode \u2192 ") +
               (play_mode == 1 ? "wave" : "notes"));
      MarkSettingsDirty();
    }
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip("Click to toggle Notes / Wave");
    }
    ImGui::SameLine(0.f, S(8.f));
  }
  ...
  // VOICES chip
  {
    ...
    char v[16];
    std::snprintf(v, sizeof(v), "%d/16", active);
    ChipSeg segs[2];
...
void App::DrawActivityLog()
{
  ...
  // Filter
  {
    ...
    ImGui::PushFont(fm);
    ImGui::InputTextWithHint("##logfilter", "filter log…", log_filter,
                             sizeof(log_filter));
    ImGui::PopFont();
    ImGui::SetCursorPosY(ImGui::GetCursorPosY() + S(4.f));
  ...
  // Console block height (target tabs + input + quick sends)
  const float console_h = S(92.f);
  ...
  if (log_view.empty()) {
    ...
    const float tw =
        fs->CalcTextSizeA(fs->FontSize, FLT_MAX, 0.f, "no entries").x;
    ImGui::SetCursorPosX((ws.x - tw) * 0.5f);
    Mono("no entries", kPalette.muted, fs);
  }
  if (log_auto_scroll &&
  ...
  const bool bus_online = bus.IsOpen() && !bus.BusFault();
  {
    ...
    std::snprintf(prompt, sizeof(prompt), "%c>",
                  raw_target == 1 ? 'E' : raw_target == 2 ? 'A' : 'C');
    ...
    const bool enter = ImGui::InputTextWithHint(
        "##cmd", bus_online ? "\u2191\u2193 history · Enter send"
                            : "bus offline",
...
void App::DrawPerform()
{
  ...
  ImGui::BeginChild("voice_grid", ImVec2(S(224.f), 0), ImGuiChildFlags_None);
  {
    ...
    // 4×4 grid — id / note / Hz when active
    const float gap = S(4.f);
    const float pad = S(8.f);
    const float cell_w = (wsz.x - pad * 2.f - gap * 3.f) / 4.f;
    ...
    if (selected_voice >= 0 && selected_voice < 16) {
      const float fy = wp.y + S(32.f) + 4.f * (cell_h + gap) + S(2.f);
  ...
  ImGui::BeginChild("piano_area", ImVec2(0, 0), ImGuiChildFlags_None);
  {
    ...
    static const BlackDef kBlacks[5] = {
        {0.65f, 1, "w"}, {1.65f, 3, "e"}, {3.65f, 6, "t"},
        {4.65f, 8, "y"}, {5.65f, 10, "u"},
    };
...
void App::DrawTone()
{
  ...
  ImGui::BeginChild("mode_bar", ImVec2(0, S(40.f)), ImGuiChildFlags_None);
  {
    ...
    Mono("PLAYBACK MODE", kPalette.text_dim, fs);
    ImGui::SameLine(0.f, S(12.f));
    ImGui::SetCursorPosY(mid_y);
    if (ModeBtn("NOTES", play_mode == 0) && play_mode != 0) {
      play_mode = 0;
      if (bus.IsOpen()) {
        bus.QueueMode(protocol::PlayMode::Notes);
      }
      log.Push(LogKind::Tx, "tx mode notes");
      MarkSettingsDirty();
    }
    ImGui::SameLine(0.f, S(6.f));
    ImGui::SetCursorPosY(mid_y);
    if (ModeBtn("WAVE", play_mode == 1) && play_mode != 1) {
      play_mode = 1;
      if (bus.IsOpen()) {
        bus.QueueMode(protocol::PlayMode::Wave);
      }
      log.Push(LogKind::Tx, "tx mode wave");
      MarkSettingsDirty();
    }
    if (offline) {
      ImGui::SameLine(0.f, S(18.f));
...
void App::DrawSetup()
{
  ...
  if (fw::ui::ChipBtn("Ping", false, BtnKind::Neutral)) {
    const auto r = bus.QueueExec(protocol::Target::Channel, "h");
    NotifyEnqueue(r == BusQueueResult::Ok, QueueResultMsg(r));
  }
  ImGui::EndDisabled();
...
void App::Draw()
{
  ...
  if (!io.WantTextInput) {
    ...
    if (ImGui::IsKeyPressed(ImGuiKey_5)) { view = GuiView::Setup; MarkSettingsDirty(); }
