...
#define RS485_ECHO 0 /* default off: upstream note bursts run at wire speed */

static uint8_t led_show_on = 1; /* LED flashing enable flag */
/** Runtime RS485 keystroke echo (bus re-TX). Default from RS485_ECHO.
 * The host disables it for the session (`ec 0`) so Channel note frames
 * can burst at wire speed without colliding with our echo. */
 ...
 *
 * Frame must hold the longest single reply — an over-long reply is
 * truncated rather than dropped: a clipped line still terminates the
 * host's exchange, whereas a silent drop looked like a dead bus (it
 * swallowed the whole boot banner). Multi-line output (the banner)
 * is sent one Reply per line. */
void EffectConsole_Reply(const char *s) {
  ...
  RS485_BusAcquire();
  if (HAL_UART_Transmit(&huart4, (const uint8_t *)frame,
                        (uint16_t)(tag_len + body_len), 30) != HAL_OK) {
    rs485_tx_fail++;
  }
  RS485_BusRelease();
}
