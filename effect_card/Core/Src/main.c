 ...
 *   SAI1_B (slave)  <- ADC1 (0x4C, PE3)  = u 1..4  (TDM slots 0..3)
 *   SAI1_A (master) <- ADC2 (0x4D, PE6)  = u 5..8  (TDM slots 0..3)
 *
...
int main(void) {
  ...
  /* One Reply per line: each RS485 frame is capped well below the full
   * banner length, and per-line sends survive truncation individually. */
  EffectConsole_Reply("\r\n");
  EffectConsole_Reply("**************************************************\r\n");
  EffectConsole_Reply("*  Effect Card [E] ready  (multi-drop RS485)     *\r\n");
  EffectConsole_Reply("*  Type 'help' or 'e:help' for the console menu. *\r\n");
  EffectConsole_Reply("**************************************************\r\n");
