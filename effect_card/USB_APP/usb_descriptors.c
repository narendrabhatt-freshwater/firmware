...
tusb_desc_device_t const desc_device = {
    ...
    .idVendor = 0xCafe, /* placeholder VID/PID; assign production values
                           before release */
    /* PID bumped (0x4014 -> 0x4015) when the audio EP size reverted to
     * the computed 388 B.  Windows caches descriptors per VID/PID, so a
     * changed endpoint under a reused PID serves stale data (the same
     * mechanism that left the Channel Card stuck at a stale volume).
     * Bump on ANY descriptor change. */
    .idProduct = 0x4015,
    .bcdDevice = 0x0100,
