 ...
 *
 * Audio data path (bring-up stage): a synthetic ramp is streamed so the
 * USB pipe can be verified end-to-end from the PC before the SAI DMA
 * capture is wired in.  The SAI ISR will later overwrite
 * usb_audio_frame[] with real ADC samples of the channel selected by
 * usb_adc_ch ("usb ch <1..8>" console command).
 */
