...
int Stub(bvm *vm){be_pushnil(vm);be_return(vm);}
...
bool SourceIsIsolated(const std::string &source){
  ...
  while(std::getline(lines,line)){
    ...
    if(line.compare(0,16,"def on_note_on()")&&line.compare(0,17,"def on_note_off()")&&
       line.compare(0,17,"def on_ramp_end()")&&line.compare(0,3,"end"))return false;
  }
  return true;
}
}
...
CompileResult BerryCompiler::CompileChannel(const std::string &source) const {
  if(!SourceIsIsolated(source))return Error("top-level script globals/modules/classes are not allowed");
  bvm *vm=be_vm_new();if(!vm)return Error("could not create Berry compiler VM");RegisterAbi(vm);
  if(be_loadbuffer(vm,"channel.be",source.data(),source.size())!=BE_OK){be_vm_delete(vm);return Error("Berry syntax or ABI error");}
  ...
  for(const char *name:handlers){bool ok=be_getglobal(vm,name)&&be_isfunction(vm,-1);be_pop(vm,1);if(!ok){std::remove(temp.c_str());be_vm_delete(vm);return Error(std::string("missing handler: ")+name);}}
