...
void SampleUacOut::BindMixer(SampleDryMixer &mixer)
{
  mixer_ = &mixer;
}
...
bool SampleUacOut::Start(const std::string &name_needle, std::string &err)
{
  ...
  for (unsigned id : ids) {
    ...
    if (info.outputChannels < kUacChannels) {
      continue;
    }
    if (!seen.empty()) {
      seen += ", ";
  ...
  RtAudioErrorType e = impl_->dac.openStream(
      &params, nullptr, RTAUDIO_SINT16, kSampleRateHz,
