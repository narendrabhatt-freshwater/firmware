# midi_bridge — MIDI keyboard → speakers or Channel Card

PC-side C++17 app: MIDI in (RtMidi), 8-voice FIFO note bank on the **host**,
...
strict ACK ([`protocol/libs/cardproto`](../libs/cardproto) + [`protocol/libs/cardlink`](../libs/cardlink)).
...
| `STEAL` | Bank full (8); oldest note dropped before new `ON` |
...
through the same 8-voice bank. The first MIDI **Note On** (or All Notes Off)
...
`rs485_console` — see [`docs/reference/rs485_console_architecture.md`](../../docs/reference/rs485_console_architecture.md).
...
ON     slot=0  note=69  A4   freq=440.00 Hz  active=1/8
```
