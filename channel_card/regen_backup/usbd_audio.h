...
/* 1 channel (mono) × 4 bytes (32-bit) per sample × samples per ms + 16 bytes headroom for sync */
#define AUDIO_OUT_PACKET                              (uint16_t)(((USBD_AUDIO_FREQ * 1U * 4U) / 1000U) + 16U)
...
typedef struct
{
  ...
  uint8_t len;
  uint8_t unit;
  uint8_t cs;       /* Control Selector: 0x01=Mute, 0x02=Volume */
...
typedef struct
{
  ...
  int16_t  vol_cur;    /* Current volume in 1/256 dB (UAC1 format) */
  int16_t  vol_min;    /* Minimum volume: −127.5 dB = 0x8100 */
  int16_t  vol_max;    /* Maximum volume: 0 dB = 0x0000 */
  int16_t  vol_res;    /* Volume resolution: 0.5 dB = 0x0080 */
  uint8_t  mute_cur;   /* Current mute state */
