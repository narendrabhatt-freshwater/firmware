...
static void Fill(int16_t *samples, int16_t origin)
{ uint32_t i; for (i=0u;i<USB_STREAM_UAC_BODY_SAMPLES;++i) samples[i]=(int16_t)(origin+(int16_t)i); }

int main(void)
{
  int16_t old_body[USB_STREAM_UAC_BODY_SAMPLES];
  int16_t new_body[USB_STREAM_UAC_BODY_SAMPLES];
  int16_t newer_body[USB_STREAM_UAC_BODY_SAMPLES];
  int16_t sample=0;
  StreamRing_Write_t write;
  int16_t packet[USB_STREAM_UAC_PACKET_WORDS] = {0};
  Fill(old_body,100); Fill(new_body,1000); Fill(newer_body,2000);
  StreamRing_Init();
  Check(STREAM_RING_SAMPLES==12240u,"normal capacity must be restored");
  ...
  Check(StreamRing_StartNote(0u)==0&&StreamRing_GetRel(0u,0u,&sample)==0&&sample==100,"promotion must start at pending origin");
  ...
  Check(StreamRing_CurrentFill(0u)==USB_STREAM_UAC_BODY_SAMPLES-20u&&StreamRing_PendingFill(0u)==USB_STREAM_UAC_BODY_SAMPLES&&StreamRing_GetRel(0u,0u,&sample)==0&&sample==120,"pending must preserve current");
  StreamRing_ArmPending(0u,12u,3u);
  Check(StreamRing_PendingFill(0u)==0u&&StreamRing_GetRel(0u,0u,&sample)==0&&sample==120,"superseding pending preserves current");
  ...
  Check(StreamRing_PendingFill(0u)==0u&&StreamRing_GetRel(0u,0u,&sample)==0&&sample==120,"discard pending preserves current");
  ...
  {unsigned i;for(i=1u;i<24u;++i)Check(StreamRing_WriteVoice(1u,4u,1u,20u,new_body,USB_STREAM_UAC_BODY_SAMPLES)==USB_STREAM_UAC_BODY_SAMPLES,"fill to exact frame credit");}
  Check(StreamRing_FreeLevel(1u)==
            STREAM_RING_SAMPLES-24u*USB_STREAM_UAC_BODY_SAMPLES&&
  ...
  packet[0]=(int16_t)(USB_STREAM_TAG_BASE|(5u<<USB_STREAM_TAG_SESSION_SHIFT)|
                      USB_STREAM_TAG_SOF|2u);
  packet[1]=(int16_t)0xFFFEu;
  Fill(packet+2,3000);
  ...
  packet[1]=(int16_t)0xFFFFu;
  packet[0]=(int16_t)(USB_STREAM_TAG_BASE|(6u<<USB_STREAM_TAG_SESSION_SHIFT)|2u);
